<template>
  <UContainer class="mb-6 px-0">
    <div class="w-full max-w-7xl pt-4 flex flex-col items-center">
      <Search default-search-type="commander" show-about class="mt-6 w-full" />
      ...
      <SearchResults :is-loading="isLoading" :search-results="searchResults" :query-param="seoEntry.query"
        :skeleton-count="40" :error-message="searchError?.message" help-text="Loading commander results..." />
    </div>
  </UContainer>
  <BackToTop />
</template>
...
const wordSearch = computed(() =>
  WordSearchSchema.parse({
    ...
    limit: 40,
    filters: { ...seoEntry.filters, isCommander: true },
