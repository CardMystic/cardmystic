  ...
  <UContainer class="mb-10 max-w-full">
    <div class="w-full flex flex-col items-center">
      <SearchSEOTitleAndDescription
        fallback-title="Search Users"
        fallback-description="Find other CardMystic users by their username."
      />
    </div>
    ...
    <div
      v-if="hasSearched && !error && !isLoading && users.length === 0"
      class="empty-state"
    >
...
const { users, totalCount, totalPages, isLoading, isFetching, error, refetch } =
  useUserSearch(debouncedQuery, page, pageSize);
