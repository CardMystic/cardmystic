...
export const BoardSchema = z.enum(["Mainboard", "Sideboard", "Considering"]).describe("The board for the card");
...
export const AddCardsToListByNameSchema = z.object({
  ...
  cardNames: z.array(z.string().min(1)).min(1).describe("Array of card names to add to the list"),
  board: BoardSchema.default("Mainboard").describe("The board to add cards to. Defaults to Mainboard"),
});

export type AddCardsToListByNameRequest = z.infer<typeof AddCardsToListByNameSchema>;
...
export type AddCardsToListByNameResponse = z.infer<typeof AddCardsToListByNameResponseSchema>;

// ---- Add Cards By Oracle IDs ----

export const AddCardsByOracleIdsSchema = z.object({
  listId: z.guid().describe("The ID of the card list to add cards to"),
  oracleIds: z.array(z.uuid()).min(1).describe("Array of Scryfall oracle IDs to add (each adds 1 copy)"),
  board: BoardSchema.default("Mainboard").describe("The board to add cards to. Defaults to Mainboard"),
});

export type AddCardsByOracleIdsRequest = z.infer<typeof AddCardsByOracleIdsSchema>;
...
export type AddCardsByOracleIdsResponse = z.infer<typeof AddCardsByOracleIdsResponseSchema>;
...
export const BulkEditBoardEntrySchema = z.object({
  ...
  cards: z.array(BulkEditCardSchema).min(1).describe("Cards in this board group"),
});
...
export const SetCommanderSchema = z.object({
  ...
  commanderName: z.string().min(1).max(200).describe("The commander card name to set"),
});

export type SetCommanderRequest = z.infer<typeof SetCommanderSchema>;

export const SetCommanderResponseSchema = z.object({
  commanders: z.array(z.string()).describe("Current commander name(s) after the operation"),
  partnerKept: z.boolean().describe("Whether the existing commander was kept as a partner"),
});

// ---- Create List ----

export const CardListFormatSchema = CardFormat.describe("The format for the card list");

export const CreateListSchema = z.object({
  name: z.string().min(1).max(200).describe("The name of the card list"),
  description: z.string().max(1000).optional().describe("Optional description for the list"),
  format: CardListFormatSchema.default("Commander").describe("The format for the list. Defaults to Commander"),
  commanders: z.array(z.string().min(1).max(200)).max(2).default([]).describe("Optional commander name(s) to set"),
});
...
export const CreateListResponseSchema = z.object({
  ...
  description: z.string().nullable().describe("The description of the created list"),
...
export const UpdateFormatResponseSchema = z.object({
  ...
  commandersCleared: z.boolean().describe("Whether commanders were cleared due to format change"),
});
...
export const UpdateNumCopiesSchema = z.object({
  listId: z.guid().describe("The ID of the card list"),
  cardName: z.string().min(1).max(200).describe("The card name to update copies for"),
  numCopies: z.number().int().min(1).max(100).describe("The new number of copies (1-100)"),
...
export const SavePrimerSchema = z.object({
  listId: z.guid().describe("The ID of the card list"),
  text: z.string().max(200_000).describe("Markdown content for the primer (max 200 000 chars)"),
});
...
export const DecklistSummarySchema = z.object({
  ...
  avatar_card_name: z.string().nullable().describe("Card name used as the list's avatar"),
  commanders: z.array(z.string()).describe("Commander card names (Mainboard, is_commander=true)"),
  color_ratios: ColorRatiosSchema.describe("Color distribution of cards in the deck (ratios sum to 1)"),
  updated_at: z.string().nullable().describe("Last update timestamp (ISO 8601)"),
  created_at: z.string().describe("Creation timestamp (ISO 8601)"),
  visibility: z.string().describe("The visibility of the list (e.g., public, private)"),
...
export type GetFeaturedDecklistsQuery = z.infer<typeof GetFeaturedDecklistsQuerySchema>;

export const GetFeaturedDecklistsResponseSchema = z.object({
  decklists: z.array(DecklistSummarySchema),
});

export type GetFeaturedDecklistsResponse = z.infer<typeof GetFeaturedDecklistsResponseSchema>;
...
export const GetActiveUserDecklistsQuerySchema = z.object({
  limit: z.coerce
    ...
    .max(200)
    .default(100)
    .describe("Maximum number of active user decklists to return (default 100)"),
});

export type GetActiveUserDecklistsQuery = z.infer<typeof GetActiveUserDecklistsQuerySchema>;

export const GetActiveUserDecklistsResponseSchema = z.object({
  decklists: z.array(DecklistSummarySchema),
});

export type GetActiveUserDecklistsResponse = z.infer<typeof GetActiveUserDecklistsResponseSchema>;

export const SearchDecklistsQuerySchema = z.object({
  query: z.string().min(1).max(200).describe("Search keywords matched against decklist name and description"),
  limit: z.coerce
    .number()
    .int()
    .min(1)
...
export type SearchDecklistsResponse = z.infer<typeof SearchDecklistsResponseSchema>;
