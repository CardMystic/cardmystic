...
export const PublicProfileSchema = z.object({
  ...
  avatar_card_name: z.string().nullable().describe("Card name used as the user's avatar"),
...
export const SearchUsersQuerySchema = z.object({
  query: z.string().min(1).max(100).describe("Search keywords matched against display name"),
  limit: z.coerce.number().int().min(1).max(50).default(20).describe("Maximum number of users to return"),
});
