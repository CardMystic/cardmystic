<template>
  <div>
    <!-- Loading State -->
    <div
      v-if="isLoadingHistory || isLoadingCards"
    ...
    <div v-else>
      ...
      >
        <Card
          ...
          :key="card.id"
          :card="{ card_name: card.name, card_data: card }"
          :show-card-info="true"
      ...
      <!-- Show More Button -->
      <div
        v-if="safeCards.length > 10 && !showAll"
      ...
      <template #content>
        <div class="p-4 space-y-4">
          <p class="text-gray-600 dark:text-gray-400">
            Are you sure you want to clear all card history? This action cannot
            be undone.
          </p>
...
const { cardHistory, isLoadingHistory, historyError, clearAllHistoryMutation } =
  useCardHistory();
...
const safeCards = computed(() =>
  Array.isArray(cards.value) ? cards.value : [],
);
