        ...
        >
          <div class="preview-sticky">
            <HoveredSearchResultPreview
              ...
              :is-commander="previewCardIsCommander"
              :is-searched="previewIsSearched"
              ...
              :partner-index="previewPartnerIndex"
              @flip="handleCardFlip"
            />
          </div>
        </aside>
        ...
        <div class="min-w-0 flex-1">
          ...
          >
            <div
              ...
              @focusin="setPreviewCard(searchedCard)"
    ...
    <template v-else-if="!queryParam">
      <div>
        <UAlert
          ...
          :description="props.helpText || 'Please enter a search above.'"
          class="mb-4"
        />
      </div>
    </template>
...
const GroupBy = defineAsyncComponent(
  () => import('~/components/search/GroupBy.vue'),
);
...
const hasPopularity = computed(
  () =>
    !!props.searchResults?.some(
      (c) => c.popularity !== undefined && c.popularity > 0,
    ),
);
...
const sortedResults = computed(() => {
  ...
  if (props.isSimilaritySearch && !props.hideSearchedCard) {
    ...
    const sortedRest = sortSearchResults(
      restCards,
      sortBy.value,
      sortDirection.value,
    );
...
const previewIsSearched = computed(() => {
  if (!previewCard.value || !props.isSimilaritySearch || props.hideSearchedCard)
    return false;
...
watch(
  () => previewCard.value?.card_data.id,
  () => {
