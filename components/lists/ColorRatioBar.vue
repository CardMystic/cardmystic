<template>
  ...
  >
    <div
      ...
      :key="segment.key"
      class="relative h-full first:rounded-l-full last:rounded-r-full group/segment"
      :style="{ width: `${segment.percent}%`, backgroundColor: segment.color }"
    >
      <!-- Hover tooltip: mana icon + percentage -->
      <div
        class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover/segment:flex items-center gap-1 px-1.5 py-0.5 rounded-md text-xs font-medium bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900 whitespace-nowrap z-20 shadow-md"
        role="tooltip"
      >
