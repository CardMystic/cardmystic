  ...
  <UModal v-model:open="isOpen" title="Create New Decklist">
    <template #content>
      <div class="p-4 space-y-4">
        ...
        <UFormField
          v-if="isCommanderFormat && showPartnerField"
          label="Partner Commander (optional)"
        >
          <UInputMenu
            v-model="newListPartnerCommander"
...
const filteredCommanders = computed(() => {
  if (
    ...
    commanderSearchTerm.value.length < 2 ||
    !commanders.value
  )
    return [];
  const search = commanderSearchTerm.value.toLowerCase();
  return commanders.value
...
const filteredPartners = computed(() => {
  ...
  return validPartnerList.value
    .filter(
      (c) => c !== newListCommander.value && c.toLowerCase().includes(search),
    )
    .slice(0, 100);
});
...
const handleCreate = async () => {
  ...
  try {
    ...
    if (newListPartnerCommander.value)
      commandersList.push(newListPartnerCommander.value);
    ...
    toast.add({
      title:
        cardCount > 0
          ? `List created and ${cardCount} card${cardCount === 1 ? '' : 's'} added`
          : 'List created',
      icon: 'i-lucide-check',
    });
    emit('success');
