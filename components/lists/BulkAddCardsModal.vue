      ...
      <div class="p-4 space-y-4">
        <UTabs :items="tabs" class="w-full">
          <template #mainboard>
            <div class="pt-3 space-y-1">
              <p class="text-xs text-gray-500 dark:text-gray-400">
                {{ boardCardCount(mainboardText) }} card{{ boardCardCount(mainboardText) === 1 ? '' : 's' }}
              </p>
              <UTextarea v-model="mainboardText" placeholder="4 Lightning Bolt&#10;2 Counterspell&#10;Sol Ring"
          ...
          <template #sideboard>
            <div class="pt-3 space-y-1">
              <p class="text-xs text-gray-500 dark:text-gray-400">
                {{ boardCardCount(sideboardText) }} card{{ boardCardCount(sideboardText) === 1 ? '' : 's' }}
              </p>
              <UTextarea v-model="sideboardText" placeholder="2 Negate&#10;1 Pithing Needle" :rows="10"
                class="w-full" />
            </div>
          </template>
          <template #considering>
            <div class="pt-3 space-y-1">
              <p class="text-xs text-gray-500 dark:text-gray-400">
                {{ boardCardCount(consideringText) }} card{{ boardCardCount(consideringText) === 1 ? '' : 's' }}
              </p>
              <UTextarea v-model="consideringText" placeholder="Ragavan, Nimble Pilferer&#10;Force of Will" :rows="10"
                class="w-full" />
            </div>
          </template>
        </UTabs>
        ...
        <div class="flex gap-2 justify-end">
          ...
          <UButton class="cursor-pointer" color="primary" variant="solid" label="Update List" :loading="bulkEditLoading"
            :disabled="totalCardCount === 0" @click="handleBulkEdit" />
        </div>
      </div>
    </template>
  </UModal>
...
const totalCardCount = computed(
  () => boardCardCount(mainboardText.value) + boardCardCount(sideboardText.value) + boardCardCount(consideringText.value),
)
