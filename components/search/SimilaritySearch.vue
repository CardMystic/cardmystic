    ...
    <UFormField name="card_name" class="mb-2">
      <div class="flex gap-2">
        <UInputMenu
          ...
          @update:model-value="handleCardSelected"
        />
        <UButton
          icon="i-mdi-cards-outline"
...
const cardNameParam = computed(() =>
  String(pageDefaults?.query ?? route.query.card_name ?? ''),
);
import { hasAdvancedFilters } from '~/utils/quickFilters';

const parsedFilters = computed(() => {
  const base: Record<string, any> = {
    selectedColorFilterOption: 'Match Exactly' as 'Match Exactly',
  };
...
const status = computed(() =>
  cardNamesStatus.value === 'pending' ? 'pending' : 'success',
);
